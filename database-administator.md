...
### Data Replication (Local -> Cloud)
When you deploy, your cloud database will begin empty. Use these methods to replicate your local data:

**Option 1: Full Database Dump (Recommended for Initial Load)**
This copies your entire local database (users, products, orders) to the cloud.
```bash
# 1. Export Local Data
mysqldump -u [local_user] -p [local_db_name] > initial_backup.sql

# 2. Import to Cloud
mysql -h [cloud_host] -u [cloud_user] -p [cloud_db_name] < initial_backup.sql
```

**Option 2: Data Seeding (For Reference Data)**
For static data like Categories or Roles, create a `src/main/resources/data.sql` file. Spring Boot executes this on startup.
- **Pros**: Version controlled, consistent across environments.
- **Cons**: Not suitable for large datasets or dynamic user data.

### Ongoing Maintenance Loop
To "stay in the loop" with your production database, monitor these key metrics:
1.  **Health**: Check the Actuator endpoint `/actuator/health` periodically. It usually reports `UP` if the DB connection is healthy.
2.  **Performance**: Monitor `HikariCP` pool usage. If `active-connections` consistently hits your max (10), consider increasing the pool size.
3.  **Backups**: Ensure your cloud provider (AWS RDS, Azure SQL, etc.) has **automated daily backups** enabled. Do not rely solely on manual dumps.
